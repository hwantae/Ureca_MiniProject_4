...
public class ReservationValidator {
    ...
    // 과거 시간 슬롯인지 검사
    public void validateNotPastSlot(TimeSlot timeSlot, java.time.LocalDate targetDate) {
         LocalDateTime slotStartDateTime =
                 LocalDateTime.of(targetDate, timeSlot.getStartTime());
    ...
    // 취소 가능 시간(시작 30분 전까지)
    public void validateStartTimeForCancel(TimeSlot timeSlot, java.time.LocalDate targetDate) {
         LocalDateTime slotStartDateTime =
                 LocalDateTime.of(targetDate, timeSlot.getStartTime());
